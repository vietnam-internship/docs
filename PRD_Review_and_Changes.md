...
|---|---|---|---|
| 한국수출입은행 환율정보 Open API | 환율 기준값 배치 수집 (9.2, 11장, 17장) | [공공데이터포털 - 한국수출입은행 환율 정보](https://www.data.go.kr/data/3068846/openapi.do) | 2025-06-25부로 요청 도메인이 `www.koreaexim.go.kr` → `oapi.koreaexim.go.kr` 로 변경됨(기존 도메인은 2026-04-30 종료 예정) — 개발 착수 전 신규 도메인 기준으로 연동해야 함. 포털 메타데이터상 갱신주기는 "실시간"으로 표기되어 있으나, 실제로는 영업일 1회(약 11시) 고시되는 일일 기준환율이므로 1장에서 지적한 "실시간" 문구 이슈는 유효함 |
